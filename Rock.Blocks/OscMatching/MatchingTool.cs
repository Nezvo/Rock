...
namespace Rock.Blocks.OscMatching
{
    ...
    public class MatchingTool : RockBlockType
    {
        ...
        public async Task<BlockActionResult> AssignOnSiteCoordinator(int projectId, int oscId)
        {
            ...
            return await AddGroupMember(projectId, oscId);
        }
        ...
        private async Task<Dictionary<int, OnSiteCoordinatorAttributeModel>> GetOnSiteCoordinatorAttributeValues(List<int> oscIds)
        {
            var attributeGuids = new List<Guid?>
            {
                _oscCampusAttribute,
            ...
            return await _attributeValueService
                ...
                .GroupBy(x => x.EntityId)
                .ToDictionaryAsync(x => x.Key.Value, x => new OnSiteCoordinatorAttributeModel
                {
        ...
        private async Task<BlockActionResult> AddGroupMember(int projectId, int oscId)
        {
            ...
            var assignedOscProjectIds = (await GetAssignedOnSiteCoordinators(_projectGroupGuid, new List<int> { oscId }))
            ...
            {
                return ActionBadRequest("Person is already assigned to this project");
            }

            var projectAttributes = await GetProjectAttributeValues(assignedOscProjectIds.Concat(new List<int> { projectId }).ToList());
            var oscAttributes = await GetOnSiteCoordinatorAttributeValues(new List<int> { oscId });
            var maxProjects = oscAttributes.GetValueOrNull(oscId)?.MaxProjects?.ToIntSafe(-1) ?? -1;

            if (maxProjects >= 0 && assignedOscProjectIds.Count >= maxProjects)
            {
                return ActionBadRequest("Person has reached the maximum number of projects");
            }
            ...
            if (primaryProject != null)
            {
                foreach (var assignedOscProjectId in assignedOscProjectIds)
                {
                    ...
                    {
                        return ActionBadRequest($"Person is already assigned to a project Id {assignedOscProjectId} that overlaps with this project");
                    }
                }
            }

            var groupRole = await _groupTypeRoleService
                .Queryable()
        ...
        private void InitServices()
        {
            ...
            _blockService = new BlockService(RockContext);
